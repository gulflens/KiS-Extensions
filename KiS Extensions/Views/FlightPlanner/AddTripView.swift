...
struct AddTripView: View {
    ...
    @State private var sectors: [SectorDraft] = [SectorDraft()]
    ...
    init(flightToEdit: PlannedFlight? = nil) {
        ...
        if let trip = flightToEdit {
            ...
            _sectors = State(initialValue: drafts.isEmpty ? [SectorDraft()] : drafts)
        }
    }
    ...
    /// Whether the trip number looks like a special (non-standard) flight.
    /// Starts with 2 or ends with a letter (e.g. 2602, 145-B).
    private var isSpecialTrip: Bool {
        let trimmed = tripNumber.trimmingCharacters(in: .whitespaces)
        guard !trimmed.isEmpty else { return false }
        return trimmed.hasPrefix("2")
            || trimmed.last?.isLetter == true
    }
    ...
    private var isTripNumberValid: Bool {
        ...
        if isSpecialTrip {
            // Special trips: at least 3 characters, digits/letters/hyphens
            return trimmed.count >= 3
        }

        // Standard trips: exactly 4 digits starting with 6 or 7
        let digitsOnly = trimmed.filter(\.isNumber)
        return digitsOnly.count == 4
            && (digitsOnly.hasPrefix("6") || digitsOnly.hasPrefix("7"))
    }

    private var canSave: Bool {
        isTripNumberValid && sectors.allSatisfy { $0.isComplete }
    }
        ...
        Form {
            ...
            Section {
                    ...
                    labeledField("Trip no.", width: 100) {
                        TextField("6001", text: $tripNumber)
                            ...
                            .onChange(of: tripNumber) { _, newValue in
                                ...
                                let filtered = String(newValue.unicodeScalars.filter { allowed.contains($0) }.prefix(6))
                                if filtered != newValue {
                                    tripNumber = filtered
                                }
                                if isSpecialTrip && tripType != .special {
                                    tripType = .special
                                } else if !isSpecialTrip && tripType == .special {
                                    tripType = .turnaround
                                }
                ...
                if !tripNumber.isEmpty && !isTripNumberValid {
                    if isSpecialTrip {
                        Text("Special trip: at least 3 characters (e.g. 2602, 145-B)")
                            .foregroundStyle(.red)
                    } else {
                        Text("4 digits starting with 6 (Airbus) or 7 (Boeing)")
                            .foregroundStyle(.red)
                    }
                }
            }
    ...
    private func layoverRow(from prevIndex: Int, to nextIndex: Int) -> some View {
        ...
        HStack(spacing: 8) {
            Toggle(isOn: $sectors[nextIndex].isLayover) {
                HStack(spacing: 6) {
                    Image(systemName: sectors[nextIndex].isLayover ? "moon.zzz.fill" : "arrow.right.arrow.left")
                        .foregroundStyle(sectors[nextIndex].isLayover ? .orange : .blue)
                        .font(.caption)
                    if sectors[nextIndex].isLayover, let duration = layoverDuration(from: prevIndex, to: nextIndex) {
                        Text("Layover in \(stationLabel):  \(duration)")
                            .font(.subheadline.weight(.medium))
                            .foregroundStyle(.secondary)
                    } else if sectors[nextIndex].isLayover {
                        Text("Layover in \(stationLabel)")
                            .font(.subheadline.weight(.medium))
                            .foregroundStyle(.secondary)
                    } else {
                        Text("Transit in \(stationLabel)")
                            .font(.subheadline.weight(.medium))
                            .foregroundStyle(.secondary)
                    }
                }
    ...
    /// Trip 6602 → 0602, trip 7001 → 0001.
    private func autoFillFlightNumber() {
        let trimmed = tripNumber.trimmingCharacters(in: .whitespaces)
        guard !isSpecialTrip,
              trimmed.count == 4,
              let first = trimmed.first,
              (first == "6" || first == "7") else { return }
    ...
    private func addSector() {
        ...
        if let lastSector = sectors.last {
            ...
            newSector.isLayover = gapMinutes(from: lastSector, to: newSector) > 360
        }
        ...
        if let trip = flightToEdit {
            ...
            for (i, draft) in sectors.enumerated() {
                if let sourceID = draft.sourceSectorID, let existing = existingByID[sourceID] {
                    ...
                    existing.savedIsLayover = i > 0 ? draft.isLayover : nil
                    ...
                    new.savedIsLayover = i > 0 ? draft.isLayover : nil
                    new.parentTrip = trip
            ...
            for (i, draft) in sectors.enumerated() {
                ...
                sector.savedIsLayover = i > 0 ? draft.isLayover : nil
